...
#attrition.percent<- count(attrition.f$Attrition)

#  ggplot(attrition) +
#    geom_col(aes(Attrition)) +
#    coord_flip() +
#    labs(title = "Attrition", y = "Count") +
#    theme(axis.title.y = element_blank(),  plot.title = element_text(hjust = .5)) 
  
#  ggplot(a)+
#    aes(x= a, fill = summary(Attrition)) +
...
### use cross validatoin to find the best lambda value
test.cvfit.i<- cv.glmnet(model.glm.train, m.labels.train, 
                       family = 'binomial',
                       type.measure = 'class',
                       nlambda = 1000,
                       standardize = F) #be sure to set to FALSE since we already standardized
logit.model.i<- glmnet(model.glm.train, m.labels.train,
                     alpha = 1,
                     family = "binomial",
                     standardize = F,
                     lambda = test.cvfit$lambda.min)
...
plot(roc.perf,main="LASSO Error Rate")
